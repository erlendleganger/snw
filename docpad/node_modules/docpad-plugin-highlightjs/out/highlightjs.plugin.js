  ...
  module.exports = function(BasePlugin) {
    ...
    return HighlightjsPlugin = (function(_super) {
      ...
      HighlightjsPlugin.prototype.highlightSource = function(opts) {
        var aliases, className, config, docpad, err, escape, language, next, removeIndentation, replaceTab, result, source, transform, transforms, _i, _len, _ref1;
        docpad = this.docpad;
        source = opts.source, language = opts.language, next = opts.next;
        config = extendr.extend({}, this.getConfig(), opts.config);
        escape = config.escape, replaceTab = config.replaceTab, aliases = config.aliases, transforms = config.transforms, removeIndentation = config.removeIndentation, className = config.className;
        ...
        result = ("<pre class=\"" + className + "\"><code class=\"" + language + "\">" + result + "</code></pre>").replace(/\t/g, replaceTab);
        next(null, result);
        return this;
      };

      HighlightjsPlugin.prototype.renderDocument = function(opts, next) {
        var cacheKey, cacheResult, className, extension, file, plugin, _ref1;
